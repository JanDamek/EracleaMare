      ...
      <?php //include_partial('gallery/form_fieldset', array('gallery' => $gallery, 'form' => $form, 'fields' => $fields, 'fieldset' => $fieldset)) ?>
		...
		<div id="sf_fieldset_<?php echo preg_replace('/[^a-z0-9_]/', '_', strtolower($fieldset)) ?>">
			...
			<?php if ($field->isPartial()): ?>
		      <?php include_partial('gallery/'.$name, array('gallery' => $gallery, 'form' => $form, 'attributes' => $attributes instanceof sfOutputEscaper ? $attributes->getRawValue() : $attributes)) ?>
		    <?php elseif ($field->isComponent()): ?>
		      <?php include_component('gallery', $name, array('gallery' => $gallery, 'form' => $form, 'attributes' => $attributes instanceof sfOutputEscaper ? $attributes->getRawValue() : $attributes)) ?>
		    <?php else: ?>
